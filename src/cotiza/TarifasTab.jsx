...
const PREFIJOS_CUPON = [
  "LaUnica", "LaHistorica", "LaPionera", "La959",
  "Araucana", "AlAire", "Cerro", "Niielol",
];
...
/**
 * Tab de tarifas y cupones. Estado local mientras se edita; al apretar
 * "Guardar cambios" hace PUT al blob via /api/cotiza/save-tarifas.
 */
export default function TarifasTab({ tarifas: tarifasIniciales, token, onLogout }) {
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (!r.ok) {
        ...
        setResultado({ ok: true, msg: `Guardado · ${data.savedAt}` });
      }
    } catch (e) {
  ...
  return (
    <div>
      <p style={K({ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 16 })}>
        Última actualización: {tarifas.actualizado || "—"}
      </p>
          ...
          {f.horarios && f.horarios.map((h, hIdx) => (
            <div key={h.id} style={{ marginTop: 16 }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 10 }}>
                {h.packs.map((p, pIdx) => (
                  <label key={p.id} style={K({ display: "flex", flexDirection: "column", gap: 4, fontSize: 11, color: "rgba(255,255,255,0.55)" })}>
                    {p.label}
          ...
          {f.unidades && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10, marginTop: 12 }}>
              {f.unidades.map((u, uIdx) => (
                <label key={u.id} style={K({ display: "flex", flexDirection: "column", gap: 4, fontSize: 11, color: "rgba(255,255,255,0.55)" })}>
                  {u.label}
                    ...
                    <Cell label="Código">
                      <div style={{ display: "flex", gap: 6 }}>
                        ...
                        <button type="button" onClick={() => regenerarCodigo(i)}
                          ...
                          style={K({ background: "rgba(82,184,112,0.15)", color: "#52b870", border: "1px solid rgba(82,184,112,0.4)", borderRadius: 6, padding: "0 10px", fontSize: 14, cursor: "pointer", flexShrink: 0 })}>
                          🎲
                        </button>
                      </div>
                    </Cell>
      ...
      <div style={{
        ...
        background: "rgba(25,25,25,0.96)", backdropFilter: "blur(8px)",
        border: "1px solid rgba(82,184,112,0.3)", borderRadius: 10, padding: 14,
        ...
        <div>
          <p style={K({ fontSize: 13, color: "#fff", fontWeight: 600 })}>
            {resultado
              ? <span style={{ color: resultado.ok ? "#52b870" : "#e87171" }}>{resultado.msg}</span>
              : "Cambios sin guardar"}
          </p>
          <p style={K({ fontSize: 11, color: "rgba(255,255,255,0.4)" })}>
            Los precios se aplican apenas guardes. Afectan al cotizador en uso.
          </p>
        </div>
